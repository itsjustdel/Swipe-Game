...
//was static. Now need an instance to use coroutines
public class MeshHelper
{
    ...
    /// </param>
    public IEnumerator Subdivide(Mesh mesh, int level, GameObject gameObject, bool doYAdjust)
    {
        if (level < 2)
            yield break;
    ...
    public static Mesh SubdivideStatic(Mesh mesh, int level)
    {
        if (level < 2)
            return mesh;
